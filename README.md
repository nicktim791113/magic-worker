...
- **敵人**：從上面踩可消滅；從旁邊碰到會扣一顆 ❤️ 愛心，愛心歸零會回到這關起點。
- **道具商店**：按 `S` 或點右上角 🛒，用金幣買能力道具與造型（見下方）。
- **過關**後自動進下一關；全部破關會顯示祝賀畫面。
- **手機 / 平板**：畫面會自動出現 ◀ ▶ 跳 觸控按鈕。

## 道具商店 🛒

按 `S`（或點右上角 🛒）打開商店，用收集到的金幣購買。金幣與已買道具會**存在瀏覽器、永久保留**：

- **能力道具**（買了永久生效）：🦘 高跳鞋、💨 風之鞋、🪽 二段跳、🛡️ 護盾（每關擋一次敵人）、❤️ 多一顆愛心。
- **造型**（純裝飾）：多種顏色，還有 🌈 彩虹。

> 想改商店賣什麼、價錢多少？打開 `src/game/profile.js` 的 `SHOP_ITEMS`。
...
│  ├─ levels/levels.js         ← 關卡資料（格子座標，含敵人/移動平台，可被編輯器讀寫）
│  ├─ game/profile.js          ← 玩家檔案（金幣錢包、已購道具）＋商店道具清單
│  └─ scenes/
...
- **工具（上排）**：🟫 地形、🪙 金幣、👾 敵人、🛗 平台、🧍 起點、🚩 終點、🧽 擦除。
  - 地形 / 金幣：在格子上**點一下**或**按住拖曳**放置。
  - 👾 敵人：**拖一段**，決定牠左右巡邏的範圍。
  - 🛗 平台（移動平台）：**拖兩點**，決定它來回移動的路徑。
  - 🧍 起點 / 🚩 終點：點一下設定。🧽 擦除：清掉點到的東西。
- **動作（下排）**：
  - **▶️ 試玩**：立刻玩玩看這一關（再按 `E` 回編輯器）。
  - **◀ 關 / 關 ▶ / ➕ 關**：切換上一關 / 下一關 / 新增關卡。
  - **💾 匯出**：把你設計的所有關卡複製出來（JSON）。
  - **🗑 清空 / ↩ 重設**：清掉這一關、或全部還原成內建關卡。
  - **↶ 復原**（或 `Ctrl+Z`）：取消上一步。**➖ 短 / ➕ 長**：調整關卡長度。
